"""CLI implementation: run one scenario and print a RunReport.

Lives in the package (not just scripts/) so the loaders can be unit-tested directly without a
subprocess. scripts/run.py is a thin wrapper around main().
...
def load_model_config(name: str) -> dict:
    ...
    if name == "mock":
        return {"adapter": "mock"}
    cfg = REPO_ROOT / "config" / "tenants.yaml"
    if not cfg.exists():
...
def load_egress():
    cfg = REPO_ROOT / "config" / "egress_allowlist.yaml"
    if cfg.exists():
        data = yaml.safe_load(cfg.read_text(encoding="utf-8")) or {}
